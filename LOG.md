...
- Aggiunto `sitemaps/aliases.txt` + gestione alias nel sync: la stessa sitemap è agganciata sia a `law-tracker` sia a `europa` (fallback SLD). **Quale dei due nomi risolva davvero a runtime resta da verificare**: serve il Browser Bridge collegato (`opencli doctor` → estensione non connessa).
