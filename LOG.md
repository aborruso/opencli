...
- Aggiunto `sitemaps/aliases.txt` + gestione alias nel sync. L'alias `europa` (fallback SLD) è stato **provato e poi respinto**: verificato che con quell'alias anche `eur-lex.europa.eu` e `commission.europa.eu` si vedono servire la sitemap di law-tracker. Meglio non trovarla che darne una sbagliata. **Verificato col Browser Bridge collegato**: `opencli browser lt open .../homepage` → `sitemap.site: "law-tracker"`, `available: true`, `source: local`. Il `domain` dell'adapter batte il fallback SLD. Ancore riverificate con `opencli browser`: `/results` ha `title: Search results`, la find sul reference distingue procedura esistente (1 match) da inesistente (`semantic_not_found`).
